...
- 各キャラクターには前回比を付与し、個人を特定できるIDを含まない軽量履歴を最大168回（約1週間）保持します。
...
- `.github/workflows/watch-character-usage.yml`: 更新時刻を独立確認し、遅延時に主集計を再起動
...
`infra/cloudflare-watchdog/` にあり、JSONが55分以上古い時だけ起動します。
