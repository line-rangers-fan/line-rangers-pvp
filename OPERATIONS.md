...
8. 主集計の一時的な取得・保存・dispatch失敗は1回だけ自動再試行します。品質・整合性テスト自体の失敗は自動で基準を緩めず、安全側に停止します。
9. GitHub Pages・Production Worker同期・本番Visual Auditも、初回失敗だけ自動再試行します。再試行でも失敗した場合にだけ障害Issueを1件作成し、次の成功時に自動で閉じます。
...
1. 主集計で一時的な通信・取得・保存・dispatch失敗が起きると、Guardianが1回だけ自動再試行します。
2. GitHub Pages・Production Worker同期・本番Visual Auditも初回失敗は1回だけ自動再試行します。実行中・待機中の処理がある場合は、重複する復旧要求を追加しません。
3. それでも復旧しない場合だけ、GitHubに障害Issueが自動作成されます。
4. 200/200、比較整合性、秘密情報、既存D1/R2確認のような安全条件は自動復旧のために緩めません。
5. 比較値のように検証済み履歴から再計算できる派生情報だけは自動再構築し、その後もう一度厳格に検証してから保存・公開します。
6. 正常な200人集計が再び成功した時点で、自動復旧側は正常状態へ戻し、障害Issueがあれば閉じます。

失敗時にはActionsのdebug-artifactsへ、個人IDを含まないcollection_failure.jsonが保存されます。stageは停止段階、previous_data_retained: trueは前回データを守れたことを示します。障害Issueが出ていない単発失敗は、まず自動復旧に任せてください。
...
単発の一時障害は自動再試行するためIssueを作りません。自動再試行でも失敗した場合、または品質・整合性ゲートそのものが失敗した場合だけIssueを作ります。障害中も前回の正常公開データは維持されます。
